...
class Subscribe final : public handlers::api::ProcessorServiceBase::Component {
 public:
  ...
  Subscribe(const userver::components::ComponentConfig& config,
      ...
      : handlers::api::ProcessorServiceBase::Component(config, component_context),
        pg_cluster_(
            component_context
                .FindComponent<userver::components::Postgres>("postgres-db-1")
                .GetCluster()) { }

  void SubscribeOnSportsLine(handlers::api::ProcessorServiceBase::SubscribeOnSportsLineCall& call);
  ...
  void RetrieveChanges(std::vector<SportInfo>& sports, T& response) {
    for (auto& sport_info: sports) {
      ...
      if (sport == enumValue) { \
        auto [id, coefficient_sum] = pg_cluster_->Execute( \
            userver::storages::postgres::ClusterHostType::kMaster, \
            "SELECT COALESCE(MAX(id), 0), COALESCE(SUM(coefficient), 0) FROM processor_schema." # tableName \
            " WHERE id > $1", last_id \
        ).AsSingleRow<TupleType>(userver::storages::postgres::kRowTag); \
        last_id = id, response[#fieldName] = coefficient_sum; \
      }     

      RetrieveChange(handlers::api::Sport::BASEBALL, baseball, baseball)
      else RetrieveChange(handlers::api::AMERICAN_FOOTBALL, american_football, football)
      else RetrieveChange(handlers::api::FOOTBALL, football, soccer)
    }
  }
